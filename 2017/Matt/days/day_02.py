...
class Day02:
    ...
    row_count = len(spreadsheet)
    col_count = len(spreadsheet[0])
    ...
    def part_1(self):
        for row in range(self.row_count):
            high = self.spreadsheet[row][0]
            low = self.spreadsheet[row][0]
            for col in range(1, self.col_count):
                value = self.spreadsheet[row][col]
                if value > high:
                    high = value
                elif value < low:
                    low = value
            self.checksum = self.checksum + high - low
        print(self.checksum)
    ...
    def part_2(self):
        for row in range(0, self.row_count, 1):
            ...
            for col in range(self.col_count):
                value = self.spreadsheet[row][col]
                for test in range(self.col_count):
                    div = self.spreadsheet[row][test]
                    if value > div:
                        if value % div == 0:
                            quotient = value / div

            self.checksum = self.checksum + quotient
        print(int(self.checksum))
